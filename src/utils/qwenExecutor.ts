...
`;
  ...
  // Session handling: --resume takes priority over --session-id
  const activeSessionId = sessionId || crypto.randomUUID();
  Logger.error(`Active session ID: ${activeSessionId} (resume: ${resume ? 'yes' : 'no'})`);
  if (resume && sessionId) {
    args.push(CLI.FLAGS.RESUME, sessionId);
  } else if (activeSessionId) {
    Logger.error("Starting new session", activeSessionId);
    args.push(CLI.FLAGS.SESSION_ID, activeSessionId);
  }
  // If neither sessionId nor resume provided, no session flags used (backward compatible)
  ...
  try {
    const output = await executeCommand(CLI.COMMANDS.QWEN, args, onProgress);
    ...
    if (errorMessage.includes(ERROR_MESSAGES.QUOTA_EXCEEDED) && model !== MODELS.FLASH) {
      ...
      // Session handling for fallback
      if (resume && sessionId) {
        fallbackArgs.push(CLI.FLAGS.RESUME, sessionId);
      } else if (activeSessionId) {
        fallbackArgs.push(CLI.FLAGS.SESSION_ID, activeSessionId);
      }
      ...
      try {
        ...
        return { output: result, sessionId: activeSessionId };
      } catch (fallbackError) {
