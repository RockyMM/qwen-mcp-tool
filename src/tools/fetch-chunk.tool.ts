...
export const fetchChunkTool: UnifiedTool = {
  ...
  execute: async (args, onProgress): Promise<{ text: string; sessionId?: string }> => {
    ...
    if (!chunks) {
      return {
        text: `❌ Cache miss: No chunks found for cache key "${cacheKey}".

Possible reasons:
1. The cache key is incorrect, Have you ran ask-qwen with changeMode enabled?
...
Please re-run the original changeMode request to regenerate the chunks.`
    ...
    if (chunkIndex < 1 || chunkIndex > chunks.length) {
      return {
        text: `❌ Invalid chunk index: ${chunkIndex}
